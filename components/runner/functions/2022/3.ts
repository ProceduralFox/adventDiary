export const Day3First2022 = (input: string) => {
  let totalPrios = 0
...
export const Day3Second2022 = (input: string) => {
  const parsed = input.split(String.fromCharCode(10)) // solving the problem as I parse the input is getting a bit too messy
  ...
  for (let index = 0; index + 3 < parsed.length; index+=3) {
    ...
    for (let index = 0; index < thirdBackpack.length; index++) {
      ...
      if(repeatElements.has(element)){ // only 1 item is common to all 3 backpacks so can add priority here and move on
